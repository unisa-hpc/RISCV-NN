...
#pragma GCC unroll FACTOR1
        for (int i = 0; i < N; ++i)
        {
            __m512 vec_s = _mm512_setzero_si512();
#pragma GCC unroll FACTOR2
            for (int k = 0; k < N; k += VECTOR_ELEMENTS)
            {
                ...
                __m512i vec_a = _mm512_load_si512((__m256i *)ptr_a);
                __m512i vec_b = _mm512_load_si512((__m256i *)ptr_b);
                __m512i vec_mul = _mm512_mullo_epi32(vec_a, vec_b);
                vec_s = _mm512_add_epi32(vec_s, vec_mul);
            }
            const int32_t sum = _mm512_reduce_add_ps(vec_s);
            c[j * N + i] = sum;
        }
    }
}
