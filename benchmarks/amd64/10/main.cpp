...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < a_tensor.sizet(); i++)
    {
        if (i % 2)
            a_tensor.data_t()[i] = std::numeric_limits<float>::infinity();
    }
    ...
    if (RUN_BASELINES && RUN_SCALARS)
    {
        timer_stats tp(
            get_code_name(BENCH_ID, kernel_kind::ScalarNoAutoVec, true, 0), //"Scalar Matmul With Mul NoAutovec",
            {
                ...
                {"N", N},
                {"FLAG_AUTOTUNE_DISABLED", FLAG_AUTOTUNE_DISABLED}},
            !true);
    ...
    for (size_t i = 0; i < N * N; i++)
    {
        ...
        if (b_ptr[i] < 0)
        { // Add the magic number for the negative numbers
            v |= 0b100000;
        }
        bp_ptr[i] = v;
    }
