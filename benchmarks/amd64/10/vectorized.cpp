...
#pragma GCC unroll FACTOR2
            for (int k = 0; k < N; k += 64)
            {
                ...
                auto infinity_mask = is_not_infinity(vec_a);
                ...
                // start the splitting process. 4 vectors of 128 bits each, converted to 4 vectors of 512bits each
                b1 = _mm512_cvtepi8_epi32(_mm512_extracti64x2_epi64(vec_b, 1));
                // 2nd Pass
                ptr_a = a + j * N + k + 16; // `a` is row major
                vec_a = _mm512_load_ps(reinterpret_cast<const __m512i *>(ptr_a));
                infinity_mask = is_not_infinity(vec_a);
                vec_a_int = _mm512_castps_si512(vec_a);
                negative_mask = _mm512_cmpgt_epi32_mask(b1, negative_detection);
                b1 = _mm512_mask_sub_epi32(b1, negative_mask, b1, sign_removal);
                // shift vec_b to line up the exponents
                shifted_vec = _mm512_slli_epi32(b1, 23);
                // add exponents
                vec_sum = _mm512_mask_add_epi32(vec_a_int, infinity_mask, vec_a_int, shifted_vec);
                vec_sum = _mm512_mask_xor_epi32(vec_sum, negative_mask, vec_sum, sign_vec);
                // accumulate
                ...
                // start the splitting process. 4 vectors of 128 bits each, converted to 4 vectors of 512bits each
                b1 = _mm512_cvtepi8_epi32(_mm512_extracti64x2_epi64(vec_b, 2));
                // 3rd Pass
                ptr_a = a + j * N + k + 32; // `a` is row major
                vec_a = _mm512_load_ps(reinterpret_cast<const __m512i *>(ptr_a));
                infinity_mask = is_not_infinity(vec_a);
                vec_a_int = _mm512_castps_si512(vec_a);
                negative_mask = _mm512_cmpgt_epi32_mask(b1, negative_detection);
                ...
                // start the splitting process. 4 vectors of 128 bits each, converted to 4 vectors of 512bits each
                b1 = _mm512_cvtepi8_epi32(_mm512_extracti64x2_epi64(vec_b, 3));
                // 4th Pass
                ptr_a = a + j * N + k + 48; // `a` is row major
                vec_a = _mm512_load_ps(reinterpret_cast<const __m512i *>(ptr_a));
                infinity_mask = is_not_infinity(vec_a);
                vec_a_int = _mm512_castps_si512(vec_a);
                negative_mask = _mm512_cmpgt_epi32_mask(b1, negative_detection);
