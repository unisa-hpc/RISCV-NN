...
if [ "$flag_auto_tune" = true ]; then
  ...
  for ((n=64; n<=1024; n*=2)); do
    ...
    echo "Building for N of $n with the auto tuned best config: UNROLL_FACTOR0=$UNROLL_FACTOR0"

    #bash build.amd64.00.sh --machine=$machine "-DUNROLL_FACTOR0=$UNROLL_FACTOR0 -DN=$n $args"
  done
fi
