...
extern void rvv_matmul_floatbitmanipu_packed2_float_uint8(
    const float *__restrict__ a,
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < N * N; i++) {
        ...
        if (v > 7) {
            std::cerr << "Exponent " << v << " is too big for 4bits PoT";
            return 1;
        }
        ...
        const bool m_high = i % 2;
        if (m_high) {
            bp_ptr[i / 2] = (bp_ptr[i / 2] & 0x0F) | (v << 4);
        } else {
            bp_ptr[i / 2] = (v & 0x0F);
        }
    }

    {
        ...
        for (volatile size_t i = 0; i < RUNS; i++) {
            timer_scope ts(tp);
            rvv_matmul_floatbitmanipu_packed2_float_uint8(a_ptr, bp_ptr, c_rvv_mul_ptr);
        }
    }
    if (RUN_BASELINES && RUN_SCALARS) c_tensor_scalar.compare(c_tensor_rvv_mul);
