...
# from dsconv2d_3bits import DenseShiftConv2d3bit
from dsconv2d_8bits import DenseShiftConv2d8bit

def conv3x3(in_planes, out_planes, stride=1):
    "3x3 convolution with padding"
    return DenseShiftConv2d8bit(in_planes, out_planes, kernel_size=3, stride=stride,
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                DenseShiftConv2d8bit(self.inplanes, planes * block.expansion,
...
class ResNet_imagenet(ResNet):
    ...
    def init_model(self):
        ...
        for m in self.modules():
            ...
            if isinstance(m, DenseShiftConv2d8bit):
                m.weight_sign.data.normal_(0, 1e-3)
                for i in range(len(m.weight_shifts)):
                    m.weight_shifts[i].data.normal_(0, 1e-3)
